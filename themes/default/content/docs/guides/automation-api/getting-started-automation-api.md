...
You'll need a [Pulumi access token]({{< relref "/docs/intro/pulumi-service/access-tokens" >}}) so that your programs can store the resulting state in the Pulumi Service. The easiest way to obtain such a token is to run `pulumi login` from the command line.
