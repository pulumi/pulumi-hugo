...
You'll need a [Pulumi access token](/docs/intro/pulumi-service/accounts#access-tokens) so that your programs can store the resulting state in the Pulumi Cloud. The easiest way to obtain such a token is to run `pulumi login` from the command line.
