...
To login non-interactively in to the CLI, you will need to set the env var `PULUMI_ACCESS_TOKEN` as a build parameter when setting up the Jenkins build. To [create a new access token]({{< relref "/docs/intro/pulumi-service/access-tokens" >}}), go the [Access Tokens](https://app.pulumi.com/account/tokens) page in the Pulumi Service.
