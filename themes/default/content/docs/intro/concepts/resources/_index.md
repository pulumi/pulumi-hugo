...
- `CustomResource`: A custom resource is a cloud resource managed by a [resource provider](providers) such as AWS, Microsoft Azure, Google Cloud or Kubernetes.
- `ComponentResource`: A [component resource](components) is a logical grouping of other resources that creates a larger, higher-level abstraction that encapsulates its implementation details.
