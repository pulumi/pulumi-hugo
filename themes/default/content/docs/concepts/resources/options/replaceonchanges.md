...
The `replaceOnChanges` resource option can be combined with the [`deleteBeforeReplace`](/docs/intro/concepts/resources/options/deletebeforereplace) resource option to trigger a resource to be deleted before it is replaced whenever a given input has changes.
