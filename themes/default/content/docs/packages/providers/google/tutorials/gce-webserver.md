...
1. Open `index.js` and replace the contents with the following:
    ...
    This example uses the [@pulumi/gcp]({{< relref "/docs/reference/pkg/gcp" >}}) package to create and manage three Google Cloud resources: a [gcp.compute.Network]({{< relref "/docs/reference/pkg/gcp/compute/network" >}}) in which the virtual machine will run, a [gcp.compute.Firewall]({{< relref "/docs/reference/pkg/gcp/compute/firewall" >}}) which allows access for incoming SSH access, and a [gcp.compute.Instance]({{< relref "/docs/reference/pkg/gcp/compute/instance" >}}) which is created inside the network from the Debian 9 base image.
