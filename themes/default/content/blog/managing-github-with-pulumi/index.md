...
Enter [Pulumi Import](/docs/cli/pulumi_import/).
...
To finish up, we [unprotect the resource](/docs/cli/pulumi_state_unprotect/):
...
Note that we are calling `refresh: true` in both Workflows, which uses [Pulumi Refresh](/docs/cli/pulumi_refresh/) to make sure that the existing GitHub resources are aligned with the resource state in our Stack.
