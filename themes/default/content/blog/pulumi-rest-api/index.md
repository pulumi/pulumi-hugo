...
The first step is to [generate an API access token]({{< relref "/docs/intro/pulumi-service/access-tokens" >}}). The Pulumi Service lets you manage many access tokens (the same as what's used for `pulumi login`) and it's easiest to just create one specific to this project's purpose. This will let us revoke that token after we're done and know that there is no lingering access.
