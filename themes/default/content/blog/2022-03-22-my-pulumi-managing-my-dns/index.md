...
draft: false
meta_desc: In this aritcle, Rawkode shows how he using Pulumi to manage his DNS records for his many domains
meta_image: meta.png
...
I typically use GSuite for any of my domains that have email configured. The domain controller class we extend provides a `createMxRecord` method. We explicitly separate this from `createRecord` because we need some conveniences, such as a unique name for reach record when there are multiple with the same priority.
...
`this.resourceName(`${name}-${index}`, type)` and `name: this.recordName(name),`. We have two functions, `resourceName()` and `recordName()` that help sanitize some of our inputs.
...
Pulumi facilitates all these abstractions that allow us to make our lives easier. This is what I do with my Pulumi program's, but what do you do with yours?
