...
Azure Container Instances bills itself as an offering that lets you "Easily run containers on Azure without managing servers"
...
This lends itself particularly well to web server projects, especially at smaller scales. You can launch applications very quickly and easily by simply specifying the image you’ve built, and your application can be running in very short order.

#### Start-Up Times

ACI Containers start fast - often in a matter of seconds. This really helps the developer lifecycle and helps to create productivity when you’re developing your application.
...
From a security perspective, ACI separates your container from others running in the same environment, meaning your application will have the same level of security as if it was running on a different virtual machine. This really helps to provide peace of mind, and is yet another benefit of the serverless model - less infrastructure means less worries.

As expected from Microsoft, ACI has excellent support for Windows containers and .NET platform applications, which is lacking around other cloud providers. If you need to run windows containers, this is a huge benefit.
...
ACI has some configuration options missing which you’d expect would be there from the beginning. One glaring omission is port mapping (for example mapping public port 80 to container port 8080). This can mean modifying your container just to get it running in ACI.
...
The lack of flexibility in Azure Container Instances means that some workloads just aren’t able to run there. If you have full control over the container images you’re building, and you just want to get off the ground quickly, with minimal fuss and configuration, Azure Container Instances is a great choice.
...
Azure App Service is a more fully-featured offering for hosting applications, focusing more on WebApps and API services. It started life as several differing offerings (Azure Websites, Azure Mobile Service & Azure BizTalk) before being bundled into a single service in March 2015. Its origins as multiple solutions means it has developed tight integrations with other Azure solutions and offers many value-add features depending on your use case.

It’s important to note that because of the myriad features Azure App Service offers, running containers is only a small part of what you can do with it.

With that in mind, let’s take a look at where it shines with regards to running containers.
...
Kubernetes is a very fully featured product, and AKS is a very conformant implementation of Kubernetes, so a lot of our good and bad considerations next will be an evaluation of Kubernetes, as well as AKS.
...
Kubernetes has a steep learning curve, and AKS is no exception. A lot of the knowledge needed to run and operate Kubernetes is handled by AKS itself, but you still have to have an in-depth understanding of how Kubernetes works in order to get your container running.

In addition to this, the specification for a simple Kubernetes job can often be hundreds of lines of code. You have to learn not only how to run the container, but also understand how Kubernetes exposes services to the world.

### What we’d use it for

The flexibility and extensibility offered by Kubernetes as a platform means it’s possible to run many types of workloads. Large companies have battled tested Kubernetes (and by extension, AKS) running everything from web applications, to machine learning.
...
Azure RedHat OpenShift is an enterprise grade implementation of Kubernetes developed by Red Hat. OpenShift builds on Kubernetes by adding support, enterprise grade security and platform as a service integrations.

If the idea of using Kubernetes for your container solutions seems appealing, but you need SLA’s around support and integrations, Azure’s Red Hat OpenShift solution is a great option.
...
As you can see from this article, there are a number of options for running containers in Azure, and choosing the right one for your workload might be difficult. We’ve tried to give an overview of the current state of the world, but Microsoft continues to add options and capabilities to all these products.
