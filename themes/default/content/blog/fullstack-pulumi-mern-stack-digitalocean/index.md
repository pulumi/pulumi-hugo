...
And when you're finished experimenting, you can tear everything down in just a few seconds with a [`pulumi destroy`](/docs/cli/pulumi_destroy):
...
* Creating a second stack with [`pulumi stack init`](/docs/cli/pulumi_stack_init) and adjusting the program to make the source `branch` configurable---a `production` stack, say, designed to deploy in response to commits to a `release` branch.
