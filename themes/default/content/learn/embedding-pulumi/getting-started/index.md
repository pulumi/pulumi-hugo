...
With all of that in mind, let's imagine that the Pulumipus Boba Tea Shop, our company from the Fundamentals learning path, has been doing so well that we can expand to jump into other markets like selling tea blends or selling its boba tea automation application to other tea shops. We're now part of a cloud engineering platform team, and we need to make it easy for developers building our applications to request the infrastructure that they need through some sort of API or perhaps a website.
...
For our example application, we're going to address something that we always deal with when working across multiple datacenters, regions, or zones: Tracking bits of data related to time like timezones, time drift, and more. We'll set up a small monitoring function that reports this data on deployment without needing a lot of time, memory, or space to run (and therefore is cheap to use).
