...
<a data-swiftype-name="resource-property" data-swiftype-type="text" href="#privatednsonlyforinboundresolverendpoint_csharp" style="color: inherit; text-decoration: inherit;">Private<wbr>Dns<wbr>Only<wbr>For<wbr>Inbound<wbr>Resolver<wbr>Endpoint</a>
    ...
    <dd><p>Indicates whether to enable private DNS only for inbound endpoints. This option is available only for services that support both gateway and interface endpoints. It routes traffic that originates from the VPC to the gateway endpoint and traffic that originates from on-premises to the interface endpoint. Default is <code>false</code>. Can only be specified if private_dns_enabled is <code>true</code>.</p>
</dd></dl>
</pulumi-choosable>
</div>
